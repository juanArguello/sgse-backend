...
@RequestMapping(path = "/api/contratoventas")
public class ContratoVentaRestController {

	@Autowired
	private ContratoVentaService contratoVentaService;

	@PreAuthorize("hasAuthority('Registrar Contrato de ventas')")
	...
	@PreAuthorize("hasAuthority('Listar Contrato de ventas')")
	...
	@PreAuthorize("hasAuthority('Actualizar Contrato de ventas')")
	...
	public ResponseEntity<?> updateContratoVenta(@Valid @RequestBody ContratoVenta contratoVenta, BindingResult result,
		...
		if (contratoVentaNuevo == null) { // comprueba si existe el contrato de venta con el id ingresado
			map.put("mensaje",
					"Error: no se pudo editar, el contrato de venta ID: " + id + " no existe en la base de datos");
			return new ResponseEntity<>(map, HttpStatus.NOT_FOUND);
		}
	...
	@PreAuthorize("hasAuthority('Eliminar Contrato de ventas')")
